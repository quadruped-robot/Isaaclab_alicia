# Alicia_duo IsaacLab 迁移 — Phase 1（Reach）进度备忘

> 把原 MuJoCo + SB3-TD3 项目（`zero-robotic-arm/5. Deep_LR/`）迁移到 IsaacLab DirectRLEnv + RSL-RL PPO。
> 第一期目标：reach-only baseline 稳定收敛（夹爪锁开，仅末端到位）。第二期：加 grasp + 二阶段成功。
...
| 维度 | 原项目（Deep_LR） | 本项目（Isaaclab_alicia） |
|---|---|---|
| 模拟器 | MuJoCo 3.8 | Isaac Sim 5.1 + PhysX |
| 框架 | gymnasium 单 env + SB3 TD3 | DirectRLEnv 向量化（4096 并行）+ RSL-RL PPO |
| 资产 | `alicia_duo_arm_mujoco.xml`（含 equality 耦合）| URDF→USD 转换 + ImplicitActuator |
| 单步训练时长 | ~3 h（5M sim steps）| ~7 min（131M env steps）|
| 算法 | TD3 off-policy | PPO on-policy（rsl-rl 5.3.0）|

无法直接复用：MuJoCo `<equality>` 约束、SB3 模型权重、numpy reward 函数。
可"语义"迁移：FK 思路、reward 的相对结构、二阶段成功设计。
...
## 四、环境设计要点（`isaaclab_alicia_env.py`）
...
## 五、可调参数完整清单（`isaaclab_alicia_env_cfg.py`）
